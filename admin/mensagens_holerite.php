...
if (isset($_POST['id_recebido'])) {
    ...
    foreach (selectMENSAGENS_HOLERITE($raiz_cnpj, $id_recebido) as $informacao) {
    ...
    //Consulta para retornar itens do holerite
    foreach (selectMENSAGENS_HOLERITE($raiz_cnpj, $id_recebido) as $resultados) {
    ...
    if (empty($resprep_banco)) {
        ...
        $retorno .= '<button type="button" name="btn-resprep" id_usu="' . $id_usu_banco . '" id_im1="' . $id_recebido . '" class="btn btn-primary ml-auto btn-resprep">';
