...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        if ($encontra_mensal == 1) {
            ...
            if (preg_match('/\/[0-9]{4}/i', $competencia)) {
        ...
        if ($cnpj_consulta == $cnpjCompleto) {
            ...
            if (preg_match('/C\.?P\.?F\.?:/i', $var_text)) {
            ...
            // Captura valor líquido (texto após "Total Liquido")
            if ($encLiquidoP2 == 1) {
            ...
            // Detecção "Total Liquido" (só após CPF encontrado)
            if ($encLiquidoP1 == 1) {
            ...
            foreach (selectGESUSU_LAYOUT_id_cpf($tabela_usu, $cpf, $idEmpDefault) as $select_tabela) {
                ...
                try {
                    if ($select_tabela != 0) {
                        ...
                        // echo "Nenhum resultado encontrado para o CPF.".$cpf.".<br>";
                        if ($exibeRegistros != 0) {
                            echo "<br>CPF:" . $cpf . "<br>";
