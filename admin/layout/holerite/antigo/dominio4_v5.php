    ...
    foreach ($key->lines as $key2) {
        ...
        if ($cnpj_consulta == $cnpj_completo) {
            ...
            if ($encontra_filial == 1) {
                ...
                if (preg_match("/[0-9]{1}/", $var_text)) {
                    $cod_integracao = limpar_texto($var_text);
                } else {
            ...
            if (preg_match('/Declaro/i', $var_text)) {
                $encontra_declaro = 1;
            }



            if ($encontra_declaro  <= 3 && $encontra_declaro != 0) {
                //echo '$encontra_declaro---' . $encontra_declaro . '---<br>';
                if ($encontra_declaro == 2) {
                    $encontra_vlrliquido = 1;
                }


                if (preg_match("/[0-9]{1}/", $var_text)) {
                    //echo '$var_text=' . $var_text . '<br>';
                    $encontra_declaro++;
                } else {
                    $encontra_declaro = $encontra_declaro;
                }
            }
            ///////////////////////////////////////////////////////////////////////////////////////////////////////
            // Verifica e identifica o valor liquido
...
if (empty($dois_cpfs)) {
    ...
    if (!empty($retorno_cnpj)) {
        ...
        for ($i = 1; $i <= $contagem_cpf; $i++) {
            ...
            if (preg_match("/[0-9]{1}/", $valor_liquido)) {
