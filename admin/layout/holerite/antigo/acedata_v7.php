...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        // // Caso encontre a mensal, ele atribui valor da proxima casa para formar a competencia
        // if ($encontra_mensal == 1) {
        //     $competencia = $var_text;
        //     if (preg_match('/[0-9]{2}\/?[0-9]{2}\/?[0-9]{4}/i', $competencia)) {
        //         $competencia = trim($competencia);
        //         // echo "COMPETENCIA:" . $competencia . "<br>";
        //         unset($encontra_mensal);
        //     }
        // }


        // if (preg_match('/Mensal/i', $var_text)) {
        //     $competencia = $var_text;
        //     $encontra_mensal = 1;
        // }

        //LOCALIZAR COMPETENCIA
        // Ao encontrar o CNPJ ele executa duas vezes e pega o valor liquido apos 2 loops
        // if ($encontra_competencia != 0) {

        //     if ($encontra_competencia == 2) {
        //         $competencia = trim($var_text);
        //         // echo "COMPETENCIA:" . $competencia . "<br>";
        //         // echo "encontraCOMPETENCIA:" . $encontra_competencia . "<br>";
        //         unset($encontra_competencia);
        //     }else{
...
if (empty($dois_cpfs)) {
    ...
    if (!empty($retorno_cnpj)) {
        ...
        for ($i = 1; $i <= $contagem_cpf; $i++) {
            ...
            foreach (selectGESUSU_LAYOUT_id_cod($tabela_usu, $cpf, $id_emp_default) as $select_tabela) {
                ...
                try {
                    ...
                    die(($_SESSION["erro_importação"] = 'Não foi possível processar o documento PDF: ' . $e->getMessage()) . (header('Location:' . $erro_1)));
