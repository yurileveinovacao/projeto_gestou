...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        if ($nro_registro % 2 != 0) {
            ...
            // Caso encontre a vlrliquido, ele atribui valor da proxima casa para formar a valor liquido
            if ($encontra_vlrliquido == 1) {

                $vlr_liquido = FormatToDecimal($var_text);

                // echo "VALOR LIQUIDO:" . $vlr_liquido . "<br>";

                unset($encontra_vlrliquido);
            }

            // Identificar Mensal
            if (preg_match('/Mensal/i', $text)) {

                $encontra_mensal = 1;
            }

            // Identificar Mensalista
            if (preg_match('/Vr. Líquido/i', $text)) {
