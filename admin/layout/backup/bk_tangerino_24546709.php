...
for ($ix = 1; $ix <= $count; ++$ix) :
    ...
    */
        ...
        $v66 = strpos($var, 'Itinere:');
        $v66_1_2 = substr($var, $v66 - 28, 40);
        ...
        if (strlen($id_usu) != 0) {
            $query = 'INSERT INTO ' . $tabela1 . ' (id_emp,pis,id_usu,periodo,datinc,btotal,bsaldo,id_processamento,id_usa_inc,origem)
                VALUES (' . $id_emp_default . ',NULL,' . $id_usu . ',' . $periodo . ',' . $today . ',' . $btotal . ',' . $bsaldo . ',' . $processamento . ',' . $id_usa . ',' . $origem . ') RETURNING id_pon1 as pk;';
            $id_pon1 = pg_fetch_result(pg_query($conn, $query), 0, 'pk')
                or die(($_SESSION["erro_importação"] = pg_last_error()) . (header('Location:' . $erro_1)));
            ...
            for ($i = 0; $i < 31; ++$i) {
                if (!empty($parte_batidas[$i])) {
                    ...
                    $query = 'INSERT INTO ' . $tabela2 . ' (id_pon1,data,ent1,sai1,ent2,sai2,ent3,sai3,bsaldo,datinc)
                     VALUES (' . $id_pon1 . ',' . $data . ',' . $ent1 . ',' . $sai1 . ',' . $ent2 . ',' . $sai2 . ',' . $ent3 . ',' . $sai3 . ',' . $bsaldo . ',' . $today . ');';
                    pg_query($conn, $query)
                        or die(($_SESSION["erro_importação"] = pg_last_error()) . (header('Location:' . $erro_1)));
                }
            }
