...
$text_vis = str_replace("Ç", "C", str_replace("Õ", "O", str_replace("(", "", str_replace("ç", "c", str_replace("õ", "o", str_replace("í", "i", str_replace("á", "a", str_replace("Á", "A", str_replace("Í", "I", $_SESSION["text_vis"])))))))));
...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        if (preg_match('/Responsavel pelas Informacoes|RESPONSAVEL PELAS INFORMACOES/i', $text)) {
        ...
        if (preg_match('/[0-9]{2}\.?[0-9]{3}\.?[0-9]{3}\-?[0-9]{2}/i', $text)) {
            if ($encontra_cpf == 0) {
                ...
                $cpf = trim(preg_replace("/[^0-9]/i", "", $cpf));
                //echo "<br>CPF:" . $cpf . "<br>";
                $encontra_cpf = 1;
            }
        }
        ...
        if ($cnpj != '42747805000143') { //CNPJ SEM ANO EXERCICIO
            if ($encontra_anoexe == 0) {
                if (preg_match('/20[0-9]{2}|Exercicio\sde\s[0-9]{4}/i', $text)) {
                    // $exercicio = $text;
