...
// // Variavel CNPJ completo com replace (somente numeros)
// $raiz_cnpj_original = substr($cnpj_completo, 0, 10);
...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        if (preg_match('/[0-9]{2}\.?[0-9]{3}\.?[0-9]{3}\/?[0-9]{4}\-?[0-9]{2}/i', $var_text)) {
            $cnpj_encontrado = str_replace(".", "", str_replace("/", "", str_replace("-", "", $var_text)));

            $cnpj = str_replace(".", "", str_replace("/", "", str_replace("-", "", $var_text)));
            $cnpj = limpar_texto($cnpj);

            $raiz_cnpj_encontrada = substr($cnpj, 0, 8);

            // if ($raiz_cnpj_encontrada == $raiz_cnpj) {

            //     $raiz_cnpj = $raiz_cnpj_encontrada;
            // }
        }
        ...
        if ($raiz_cnpj_encontrada == $raiz_cnpj) {
            ...
            if ($raiz_cnpj_encontrada != '42747805') { //CNPJ SEM ANO EXERCICIO
                if ($encontra_anoexe == 0) {
                    if (preg_match('/2[0-9]{3}|Exercicio\sde\s[0-9]{4}/i', $var_text)) {
                        // $exercicio = $text;
                        preg_match('/[0-9]{4}/i', $var_text, $exercicio);
                        $anoexe = $exercicio[0];
                        $anocal = $exercicio[0] - 1;
                        // echo "<br>Exercício de 0:" . $anoexe  . "<br>";
...
// echo "<br>anocal:" . $anocal . "<br>";
// echo "<br>anoexe:" . $anoexe . "<br>";
...
if (!empty($retorno_cnpj)) {
    ...
    for ($i = 1; $i <= $contagem_cpf; $i++) {
        ...
        foreach (selectGESUSU_LAYOUT_id_cpf_raiz($tabela_usu, $cpf, $raiz_cnpj) as $select_tabela) {
            $id_usu = $select_tabela['id_usu'];
            $nome = $select_tabela['nome'];
            $cargo = $select_tabela['funcao'];
