...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        if (preg_match('/[0-9]{2}\.?[0-9]{3}\.?[0-9]{3}\/?[0-9]{4}\-?[0-9]{2}/i', $text)) {
            ...
            if ($folha_func >= 1) {
                ...
                if ($cnpj == $cnpj_completo) {
                    ...
                    foreach (selectGESUSU_LAYOUT_id_pis($tabela, $pis, $id_emp_default) as $select_tabela) {
        ...
        if (preg_match('/DE\s[0-9]{2}\/[0-9]{2}\/[0-9]{4}/i', $text)) {
            $periodo = $text;
        }
    }
}
