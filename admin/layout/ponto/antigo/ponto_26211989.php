...
foreach ($json_base->analyzeResult->readResults as $key) {
    ...
    foreach ($key->lines as $key2) {
        ...
        // Identificar CNPJ
        if (preg_match('/[0-9]{3}\.?[0-9]{3}\.?[0-9]{3}\-?[0-9]{2}/i', $text)) {
            $cpf = $text;
        ...
        if (preg_match('/DE\s[0-9]{2}\/?[0-9]{2}\/?[0-9]{4}\sATÉ\s[0-9]{2}\/?[0-9]{2}\/?[0-9]{4}/i', $text)) {
            $periodo = $text;
    ...
    if ($folha_func >= 1) {
        ...
        if ($cnpj == $cnpj_completo) {
            ...
            foreach (selectGESUSU_LAYOUT_CPF($tabela_usu, $cpf) as $select_tabela) {
