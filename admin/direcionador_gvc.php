...
$diretorio = 'uploads/gvc/' . $raiz_cnpj . '.' . $ext;
...
if ($tipo_layout == $ext) {
  ...
  if (file_exists($diretorio)) {
    ...
    unlink('uploads/gvc/' . $raiz_cnpj . '.' . $ext);

    if ($tamanho > 9000000) {
      echo "<script language=javascript>
            alert('VERIFIQUE O TAMANHO DO SEU ARQUIVO!!');
            location.href = 'recibo_pagamento_gvc';
            </script>";
      exit;
    }
    ...
    $mover = move_uploaded_file($temp, 'uploads/gvc/' . $novo_nomepdf);
    ...
    $pdfSourceFile = "uploads/gvc/" . $novo_nomepdf;

    $pdfOutFile1 = "uploads/" . $novo_nomepdf;
