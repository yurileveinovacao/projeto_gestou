...
if ((isset($_POST['id_recebido'])) and (isset($_POST['opcao']))) {
    ...
    foreach (selectINFORME_DADOS($raiz_cnpj, $id_recebido) as $resultado) {
        ...
        $retorno .= '<div class="col-md-4">';
        $retorno .= '<span style="line-height: 3.00;">' . 'Ano Calendário: ' . $resultado['anocal'] . '&nbsp; / Ano Exercício: &nbsp;' . $resultado['anoexe'] . '</span>';
        $retorno .= '</div>';
    ...
    foreach (selectINFORME_ITENS($raiz_cnpj, $id_recebido) as $resultados) {
        //Montar linhas tabela para venvimentos
        $retorno .= '<iframe style="width: 100%; height: 600px;" src="../upload/beneficios/irrf/' . $raiz_cnpj . '/' . $resultados["arquivo"] . '"></iframe>';
    }
