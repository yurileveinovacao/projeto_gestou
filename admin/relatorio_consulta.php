    ...
    foreach (select_GRELAT($id_emp_default) as $row) {
        ...
        if ($row['parent_id'] != 0) {
            ...
            //FILHO
            $sub_data['text'] = "<input type='hidden' id='codigo_rel' value=" . $row["id_rel"] . "></input>" . $row['nome'] . "<input type='hidden' id='link_rel' value=" . $row["layout"] . "></input>";
        } else {
    ...
    $retorno = array();
    $retorno = array_merge($retorno, $data);
