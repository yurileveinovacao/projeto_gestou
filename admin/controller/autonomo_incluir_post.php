<?php
// FEA-009 Fase 2 — Inclusão de autônomo
...
try {
    ...
    if (selectGESAUT_cpf_exists($id_emp_default, $cpf)) {
        echo json_encode(['status' => 'cpf_duplicado', 'mensagem' => 'Este CPF já está cadastrado nesta empresa.']);
        exit;
    }
    ...
    $id_aut = insertGESAUT($id_emp_default, $nome, $cpf, $email, $pix, $rg, $data_nasc, $etnia,
                           $endereco, $cep, $bairro, $cidade, $uf, $id_usa_default);

    echo json_encode(['status' => 'sucesso', 'id_aut' => $id_aut]);
